#!/usr/bin/env python3
"""
Class BayesianOptimization, that performs
...
class BayesianOptimization:
    ...
    def acquisition(self):
        ...
        if self.minimize:
            Y_min = np.min(self.gp.Y)
            val = Y_min - mu - self.xsi
        else:
            Y_max = np.max(self.gp.Y)
            val = mu - Y_max - self.xsi
        Z = val.astype(float) / sigma.astype(float)
        Z[Z == np.inf] = 0
        EI = val * norm.cdf(Z) + sigma * norm.pdf(Z)
